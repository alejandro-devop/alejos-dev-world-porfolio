...
export function AboutSection({ data, locale }: AboutSectionProps) {
  const socialLinks = data.socialLinks.filter((link) => hasText(link.url));
  const showLocation = hasText(data.location);
  const showEmail = hasText(data.email);
  const showResume = hasText(data.resumeUrl);
  const showMetaCard = showLocation || showEmail || showResume;
            ...
            {showMetaCard && (
              ...
              >
                <div className="card-surface p-6 space-y-5">
                  ...
                  {showResume && (
                    <div className={showLocation || showEmail ? "pt-1" : undefined}>
                      <a
                        href={data.resumeUrl.trim()}
                        download
                        className={cn(
